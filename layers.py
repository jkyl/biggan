...
class Gain(Layer):
  ...
  def build(self, input_shape):
    self.gamma = self.add_weight(
      name='gamma', shape=[], initializer='zeros', dtype='float32', trainable=True)
...
def attention(x):
  ...
  theta = ConvSN2D(c // 8, 1, use_bias=False, kernel_initializer='orthogonal')(x)
  theta = Reshape((-1, c // 8))(theta)
  phi = ConvSN2D(c // 8, 1, use_bias=False, kernel_initializer='orthogonal')(x)
  phi = AveragePooling2D()(phi)
  phi = Reshape((-1, c // 8))(phi)
  f = Dot(2)([theta, phi])
  f = Activation('softmax')(f)
  g = ConvSN2D(c // 2, 1, use_bias=False, kernel_initializer='orthogonal')(x)
  g = AveragePooling2D()(g)
  g = Reshape((-1, c // 2))(g)
  y = Dot([2, 1])([f, g])
  y = Reshape((h, w, c // 2))(y)
  y = ConvSN2D(c, 1, use_bias=False, kernel_initializer='orthogonal')(y)
  y = Gain()(y)
  y = Add()([x, y])
  return y
...
def g_block(x, dim, first=False, last=False):
  if first:
    x = DenseSN(4 * 4 * dim, use_bias=False, kernel_initializer='orthogonal')(x)
    x = Reshape((4, 4, dim))(x)
  else:
    x0 = x
    for j in range(2):
      ...
      if j == 0:
        x = SubPixel(2)(x)
        x0 = SubPixel(2)(x0)
      x = ConvSN2D(dim, 3, padding='same', use_bias=False, kernel_initializer='orthogonal')(x)
