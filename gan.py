...
class GAN(object):
  ...
  def orthogonal_regularization(self):
    def R(W):
      ...
      return tf.reduce_sum((WTW * offdiagonal) ** 2)
  ...
  def prepare_summary(self, output_dir, **kwargs):
    ...
    for image in ['x', 'xhat']:
      if image in kwargs:
        tf.summary.image(image, data.postprocess_img(kwargs[image]), 1)
  ...
  def train(self, input_dir, output_dir,
      batch_size=16, n_D_updates=1, beta=0, **kwargs):
    ...
    with tf.name_scope('losses'):
      ...
      L_G_R = L_G + (beta * R if beta else 0)
    with tf.name_scope('optimizers'):
      G_opt = tf.train.AdamOptimizer(1e-4, 0, 0.999)\
        .minimize(L_G_R, var_list=self.G.trainable_weights)
    ...
    writer, summarize = self.prepare_summary(output_dir,
      L_G=L_G, L_D=L_D, R=R, x=x, xhat=xhat)
    ...
    with tf.Session() as sess:
      ...
      for i in progbar:
        for _ in range(n_D_updates - 1):
          sess.run(D_opt)
        if i % 10:
          ld, lg = sess.run([D_opt, G_opt, L_D, L_G])[-2:]
        else:
          ld, lg, s = sess.run([D_opt, G_opt, L_D, L_G, summarize])[-3:]
