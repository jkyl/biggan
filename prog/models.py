...
def UpConvFunc(x, dim, n=2):
    ...
    for i in range(n):
        x = tf.keras.layers.Conv2D(dim, 3, padding='same')(x)
        x = tf.keras.layers.LeakyReLU(0.2)(x)
    x_rgb = tf.keras.layers.Conv2D(3, 1, padding='same')(x)
    return x, x_rgb
...
def NVIDIA_generator():
    ...
    return tf.keras.models.Model(z, outputs[::-1])
...
def OutputLayers():
    return [
        ...
        tf.keras.layers.Conv2D(512, 4, padding='valid'),
        tf.keras.layers.LeakyReLU(0.2),
